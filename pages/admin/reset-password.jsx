...
import { AdminForgotPassGotOtp, AdminSetForgotPassEnd } from '../../utils/EndPoints'
...
const AdminResetPassword = () => {
  ...
  const sentForgotOTP = async (e) => {
    ...
    setSpinner(true)
    try {
      const res = await axios.put(AdminForgotPassGotOtp, {
        email: email,
      })
      if (res?.status === 200) {
  ...
  const requestForSetNewPass = async (e) => {
    ...
    try {
      const res = await axios.put(AdminSetForgotPassEnd, {
        email: email,
        otp: newPass?.otp,
